...
public class ConfiguredIncrementalBackupMethodTest {
        ...
        if (fake) {
            Files.walk(sourcePath).filter(path -> path.toFile().isFile()).limit(4).forEach(path -> {
                ...
                if (!path.toFile().delete())
                    fail();
                if (!Objects.equals(path.toFile().getName(), BackupMetadata.METADATA_FILENAME))
                    success[0]++;
            });
            if (success[0] == 0)
                fake = false;
    ...
    public String calcMD5HashForDir(File dirToHash, boolean includeHiddenFiles) {
        ...
        collectInputStreams(dirToHash, fileStreams, includeHiddenFiles, Collections.singleton(BackupMetadata.METADATA_FILENAME));
